...
/// Importando os paises do arquivo json para o javascript.
//var json = require('./paises-array.json');
//console.log(json)
///console.log(json[0].sigla2);
...
const pj_end_cod_pais = document.getElementById("pj_end_cod_pais")
...
window.addEventListener('load', async()=>{   
    ...
    paises.map((pais)=>{
        options.innerHTML += '<option>'+pais.sigla2+'</option>'
    })            
        
  

    pj_end_cod_pais.append(options);

});

/// tratando o arquivo json e relacionando com o nome do pais para html

const pj_pais =document.getElementById("pj_pais")
pj_end_cod_pais.addEventListener('change',async function(){   
    ...
    let options = '';
    paises.map((nome_paises)=>{
        options +='<option>'+nome_paises.nome+'</option>'
        
    });

    pj_pais.innerHTML = options;
});
